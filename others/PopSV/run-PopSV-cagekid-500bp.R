...
bin.size = 500
##
files.df = init.filenames(files.df, code="500bp")
...
submitJobs(getGC.reg, 1, resources=list(walltime="6:0:0", nodes="1", cores="1"), wait=function(retries) 100, max.retries=10)
...
submitJobs(getBC.reg, findNotDone(getBC.reg), resources=list(walltime="72:0:0", nodes="1", cores="1"), wait=function(retries) 100, max.retries=10)
showStatus(getBC.reg)


#### Sample QC and reference sample definition
ref.samples = subset(files.df, status=="normal")$sample
...
bcNormTN.f <- function(chunk.id, file.bc, file.bin, cont.sample){
  ...
  tn.norm(bc.df, cont.sample, bins=subset(bins.df, sm.chunk==chunk.id)$bin, norm="bootstrap")
}
...
submitJobs(bcNormTN.reg, findExpired(bcNormTN.reg) , resources=list(walltime="24:0:0", nodes="1", cores="1",supervisor.group="bws-221-ae"), wait=function(retries) 100, max.retries=10)
showStatus(bcNormTN.reg)

#### Write normalized bin counts and reference metrics
out.files = paste("ref-cagekid-500bp", c("bc-norm.tsv", "msd.tsv"), sep="-")
...
submitJobs(callCases.reg, findNotDone(callCases.reg), resources=list(walltime="20:0:0", nodes="1", cores="1",queue="sw", supervisor.group="bws-221-ae"), wait=function(retries) 100, max.retries=10)
...
abCovCallCases.f <- function(samp, files.df, norm.stats.f, bins.f, bin.size){
  library(PopSV, lib.loc="/home/jmonlong/R/PopSVforPaper")
  load(bins.f)
  call.abnormal.cov(files.df=files.df, samp=samp, out.pdf=paste0(samp,"/",samp,"-sdest-abCovCall.pdf"), FDR.th=.001, merge.cons.bins="stitch", z.th="sdest", norm.stats=norm.stats.f, stitch.dist=bin.size*5+1, gc.df=bins.df, min.normal.prop=.6)
}
...
submitJobs(abCovCallCases.reg, findNotDone(abCovCallCases.reg) , resources=list(walltime="2:0:0", nodes="1", cores="1",queue="sw", supervisor.group="bws-221-ae"), wait=function(retries) 100, max.retries=10)
showStatus(abCovCallCases.reg)

res.df = do.call(rbind, reduceResultsList(abCovCallCases.reg))
res.df = merge(res.df, files.df[,c("sample","status","individual")])
save(res.df, file="cnvs-cagekid-500bp-FDR001-mergeStitch-thSdest.RData")
...
abCovCallCases05.f <- function(samp, files.df, norm.stats.f, bins.f, bin.size){
  library(PopSV, lib.loc="/home/jmonlong/R/PopSVforPaper")
  load(bins.f)
  call.abnormal.cov(files.df=files.df, samp=samp, out.pdf=paste0(samp,"/",samp,"-sdest-abCovCall.pdf"), FDR.th=.05, merge.cons.bins="stitch", z.th="sdest", norm.stats=norm.stats.f, stitch.dist=bin.size*5+1, gc.df=bins.df, min.normal.prop=.6)
}
batchMap(abCovCallCases05.reg, abCovCallCases05.f, files.df$sample, more.args=list(files.df=files.df, norm.stats.f=out.files[2], bins.f="bins.RData",bin.size=bin.size))
submitJobs(abCovCallCases05.reg, findNotDone(abCovCallCases05.reg) , resources=list(walltime="2:0:0", nodes="1", cores="1",queue="sw", supervisor.group="bws-221-ae"), wait=function(retries) 100, max.retries=10)
showStatus(abCovCallCases05.reg)

res.df = do.call(rbind, reduceResultsList(abCovCallCases05.reg))
res.df = merge(res.df, files.df[,c("sample","status","individual")])
save(res.df, file="cnvs-cagekid-500bp-FDR05-mergeStitch-thSdest.RData")
...
indexExtra.f <- function(dump, files.df, bc.f, msd.f){
  library(PopSV, lib.loc="/home/jmonlong/R/PopSVforPaper")
  comp.index.files(bc.f, rm.input=FALSE, reorder=TRUE)
  comp.index.files(msd.f, rm.input=FALSE, reorder=TRUE)
  comp.index.files(subset(files.df, status=="normal")$fc, rm.input=FALSE, reorder=TRUE)
}
batchMap(indexExtra.reg, indexExtra.f, 1, more.args=list(files.df=files.df, bc.f=out.files[1], msd.f="ref-msd.tsv"))
submitJobs(indexExtra.reg, 1, resources=list(walltime="6:0:0", nodes="1", cores="1",queue="sw", supervisor.group="bws-221-ae"), wait=function(retries) 100, max.retries=10)
