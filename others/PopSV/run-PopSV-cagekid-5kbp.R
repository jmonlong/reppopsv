...
submitJobs(bcNormTN.reg, findExpired(bcNormTN.reg) , resources=list(walltime="12:0:0", nodes="1", cores="1",supervisor.group="bws-221-ae"), wait=function(retries) 100, max.retries=10)
showStatus(bcNormTN.reg)

#### Write normalized bin counts and reference metrics
out.files = paste("ref-cagekid-5kbp", c("bc-norm.tsv", "msd.tsv"), sep="-")
...
submitJobs(callCases.reg, findDone(callCases.reg)[-1], resources=list(walltime="6:0:0", nodes="1", cores="1",queue="sw", supervisor.group="bws-221-ae"), wait=function(retries) 100, max.retries=10)
...
abCovCallCases.f <- function(samp, files.df, norm.stats.f, bins.f, bin.size){
  library(PopSV, lib.loc="/home/jmonlong/R/PopSVforPaper")
  load(bins.f)
  call.abnormal.cov(files.df=files.df, samp=samp, out.pdf=paste0(samp,"/",samp,"-sdest-abCovCall.pdf"), FDR.th=.001, merge.cons.bins="stitch", z.th="sdest", norm.stats=norm.stats.f, stitch.dist=bin.size*2+1,  min.normal.prop=.6, gc.df = bins.df)
}
...
save(res.df, file="cnvs-cagekid-5kbp-FDR001-mergeStitch-thSdest.RData")
...
abCovCallCases05.f <- function(samp, files.df, norm.stats.f, bins.f, bin.size){
  library(PopSV, lib.loc="/home/jmonlong/R/PopSVforPaper")
  load(bins.f)
  call.abnormal.cov(files.df=files.df, samp=samp, out.pdf=paste0(samp,"/",samp,"-sdest-abCovCall.pdf"), FDR.th=.05, merge.cons.bins="stitch", z.th="sdest", norm.stats=norm.stats.f, stitch.dist=bin.size*2+1,  min.normal.prop=.6, gc.df = bins.df)
}
...
res.df = do.call(do.call, reduceResultsList(abCovCallCases05.reg), identity)
res.df = merge(res.df, files.df[,c("sample","status","individual")])
save(res.df, file="cnvs-cagekid-5kbp-FDR05-mergeStitch-thSdest.RData")
